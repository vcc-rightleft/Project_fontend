...
function save_money() {
    ...
    if (existingIndex !== -1) {
        ...
        let id =
        saveMoney.length > 0 ? saveMoney[saveMoney.length - 1].id + 1 : 1;
        let newsaveMoney = {
        money: money,
        month: month,
        id: id,
        ...
        document.getElementById("bug").innerHTML ="✅ Đã thêm mới số tiền cho tháng này.";
    }
...
function addButton() {
    ...
    if (contentMoney === "" || parseInt(contentMoney) < 0) {
        document.getElementById("bugContentMoney").innerHTML = "🚫Số tiền không hợp lệ!!!";
        return;
    }
    let saveMoney = JSON.parse(localStorage.getItem("saveMoney")) || [];
    let saveIndex = saveMoney.findIndex((item) => item.month === month);
    if (saveIndex === -1) {
        document.getElementById("bugContentMoney").innerHTML = "🚫 Bạn chưa đặt ngân sách cho tháng này.";
        return;
    }
    ...
    if (remaining < 0) {
        document.getElementById("bugContentMoney").innerHTML = "🚫 Số tiền không đủ.";
        return;
    }
    ...
    let id = monthCategory.length === 0 ? 1 : monthCategory[monthCategory.length - 1].categories[0].id + 1;
    let newCategory = {
        id: id,
        month: month,
...
function button_delete(id) {
    let month = document.getElementById("month").value.trim();
    let monthCategory =JSON.parse(localStorage.getItem("monthCategory")) || [];
    let index = monthCategory.findIndex((item) => item.categories[0].id === id && item.month === month);
    if (index === -1) {
        document.getElementById("bugContentMoney").innerHTML ="Không tìm thấy mục cần xóa!!!";
        return;
    }
    let confirmDelete = confirm("Bạn có chắc chắn muốn xóa mục này không?");
    if (!confirmDelete) {
        document.getElementById("bugContentMoney").innerHTML ="Đã hủy xóa mục này.";
        return;
    }
...
function renderhis(sortBy = "default", searchText = "") {
    ...
    let paginationStr = "";
    if (totalPages > 1) {
        paginationStr += `<button onclick="changePage(${currentPage - 1})" ${currentPage === 1 ? "disabled" : ""}>Previous</button>`;
        for (let i = 1; i <= totalPages; i++) {
            paginationStr += `<button onclick="changePage(${i})" class="${i === currentPage ? "active-page" : ""}">${i}</button>`;
        }
        paginationStr += `<button onclick="changePage(${currentPage + 1})" ${currentPage === totalPages ? "disabled" : ""}>Next</button>`;
    }
...
function renderMonthlyStats() {
    ...
    const spending = {};
    const monthlyStats = [];
    transactions.forEach((t) => {spending[t.month] = (spending[t.month] || 0) + parseInt(t.money);months.add(t.month);});
    saveMoney.forEach((s) => months.add(s.month));
    const tbody = document.getElementById("statsBody");
    tbody.innerHTML = "";
    [...months].sort().forEach((month) => {
    const spent = spending[month] || 0;
        const budget = parseInt((saveMoney.find((s) => s.month === month) || {}).money ||0);
...
function loadSavedStats() {
    ...
    tbody.innerHTML = "";
    stats.forEach((stat) => {
    const isExceeded = stat.status === "Vượt";
    tbody.innerHTML += `
            <tr>
                <td>${stat.month}</td>
